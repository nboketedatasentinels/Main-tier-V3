-- ============================================================================
-- T4L  ·  Require reason on partner points adjust (notify from app)
-- 0053: Partners must supply a reason. Learner in-app notification is sent by
-- the partner dashboard after a successful adjust (points + reason), so this
-- RPC no longer inserts a second notification row.
--
-- Safe to re-run (idempotent replace of partner_adjust_user_points).
-- ============================================================================
...
begin
  ...
  if v_reason is null or char_length(v_reason) < 3 then
    raise exception 'A reason is required so the learner knows why points changed';
  end if;
