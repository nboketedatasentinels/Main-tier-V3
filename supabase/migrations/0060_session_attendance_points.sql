-- ============================================================================
-- T4L  ·  Mentor / coach session attendance points
-- 0060: Allow mentors and coaches to credit mentor_meetup / ambassador_session
--       (+2,000) to a learner when they mark attendance.
--
-- WHY: award_checklist_points previously allowed only the learner or
-- partner/admin (auth.uid() = learner). Mentors/coaches marking attended
-- always got "forbidden", so session points never landed.
--
...
revoke all on function public.can_issue_session_attendance_points(uuid, text, text) from public;
grant execute on function public.can_issue_session_attendance_points(uuid, text, text) to authenticated;
...
begin
  if not (
    ...
    or public.can_issue_session_attendance_points(v_uid, v_activity_id, v_source)
  ) then
