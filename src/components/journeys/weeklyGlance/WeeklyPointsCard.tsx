        ...
        <Stack spacing={3}>
          <HStack justify="space-between">
            <HStack>
              ...
              <Text fontWeight="bold" color="#5A6ACF">Weekly Points</Text>
            </HStack>
            {data?.status && (
          ...
          <Skeleton isLoaded={!loading} rounded="md">
            <VStack align="stretch" spacing={2}>
              <HStack justify="space-between">
                <Text fontSize="xs" color="#5A6ACF">
                  Target
                </Text>
                <Text fontWeight="bold" color="#5A6ACF">{data ? `${data.target_points || 0} pts` : '--'}</Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="xs" color="#5A6ACF">
                  Earned
                </Text>
                <Text color="#5A6ACF">{data ? `${data.points_earned || 0} pts` : '--'}</Text>
              </HStack>
          ...
          {error && (
            <HStack color="red.500" fontSize="sm">
              <Icon as={AlertCircle} />
              <Text color="#5A6ACF">Unable to load weekly points.</Text>
            </HStack>
          )}
        </Stack>
      </CardBody>
