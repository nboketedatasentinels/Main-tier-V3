...
type CoachGuidelinesModalProps = {
  isOpen: boolean
  onAcknowledged: () => void
}

export const CoachGuidelinesModal: React.FC<CoachGuidelinesModalProps> = ({
  isOpen,
  onAcknowledged,
}) => {
  ...
  const handleUnderstand = async () => {
    ...
    try {
      ...
      const { error } = await updateProfile({
        coachGuidelinesAcknowledgedAt: acknowledgedAt,
      })
      if (error) {
        toast({
      ...
      try {
        localStorage.setItem('t4l.coach_guidelines_acknowledged', acknowledgedAt)
      } catch {
        // ignore
      }
      onAcknowledged()
