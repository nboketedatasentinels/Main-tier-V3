...
/**
 * Mentor signup / sign-in (route-driven, same pattern as partner auth).
 *
 * Live URLs:
 * - https://app.t4leader.com/mentor-signup
 * - https://app.t4leader.com/mentor-signin
 *
 * Authorization: admin/partner must invite or assign the email as mentor.
 * Signup then accepts pending org invitations and verifies role=mentor.
 */
export const MENTOR_ACCESS_CODE = 't4l.ds.Mentor.2025#'

export const MentorSignupPage: React.FC<{ initialMode?: 'signup' | 'signin' }> = ({
  initialMode = 'signup',
}) => {
  ...
  /** Accept pending mentor invite (if any) and verify role. */
  const activateMentorAccess = async (): Promise<boolean> => {
    ...
    // Pending admin/partner mentor invites land here.
    ...
    if (role !== 'mentor' && role !== 'super_admin') {
      await supabase.auth.signOut()
      setError(
        'This email has not been assigned as a mentor. Ask your admin or partner to invite you as a mentor, then sign up again.',
      )
      return false
    }
    return true
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (accessCode.trim() !== MENTOR_ACCESS_CODE) {
      setError('Invalid mentor access code. Please use the code from your invitation email.')
      return
    }
    setLoading(true)
    try {
      ...
      const { data, error: signUpError } = await supabase.auth.signUp({
        ...
        password,
        options: {
          emailRedirectTo: `${window.location.origin}/mentor-signin`,
      ...
      if (signUpError) {
        if (/already|registered|exists/i.test(signUpError.message)) {
          navigate('/mentor-signin', {
            state: {
              info: 'You already have an account. Sign in below to activate mentor access.',
              email: normalizedEmail,
            },
          })
          return
      ...
      if (data.session) {
        const ok = await activateMentorAccess()
        if (ok) {
          await supabase.auth.signOut()
          navigate('/mentor-signin', {
            state: {
              info: 'Your mentor account is ready. Please sign in to continue.',
              email: normalizedEmail,
            },
          })
        }
          ...
          <VStack spacing={6} align="stretch">
            <VStack spacing={2}>
              ...
              <Text fontSize="2xl" fontWeight="bold" color="#27062e" letterSpacing="-0.02em">
                {mode === 'signin' ? 'Mentor sign in' : 'Mentor sign up'}
              </Text>
              <Text fontSize="sm" color="gray.600" textAlign="center" lineHeight="1.6">
                {mode === 'signin'
                  ? 'Sign in to activate your mentor access.'
                  : 'Create your mentor account. Your email must be invited or assigned as a mentor by an admin or partner.'}
              </Text>
            </VStack>
            ...
            {mode === 'signup' ? (
              <form onSubmit={handleSignup}>
                <VStack spacing={4} align="stretch">
                  ...
                  {passwordField}
                  <FormControl isRequired>
                    <FormLabel color="gray.800">Mentor access code</FormLabel>
                    <Input
                      value={accessCode}
                  ...
                  <Button variant="ghost" size="sm" onClick={() => navigate('/mentor-signin')}>
              ...
              <form onSubmit={handleSignin}>
                <VStack spacing={4} align="stretch">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => navigate('/mentor-signup')}>
...
/** @deprecated Prefer MentorSignupPage with initialMode="signin" */
export const MentorSignInPage = () => <MentorSignupPage initialMode="signin" />
