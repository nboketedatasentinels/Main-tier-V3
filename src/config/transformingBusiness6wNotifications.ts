...
const buildEmailHtml = (params: {
  ...
  const paragraphsHtml = params.paragraphs
    .map(
      (p) =>
        `<p style="margin:0 0 14px;font-size:14px;color:#4A5568;line-height:1.6;">${p}</p>`,
    )
    .join('')
...
const buildEmailText = (params: {
  ...
  const paragraphs = params.paragraphs.join('\n\n')
  const links = params.links ? '\n\n' + params.links.map((l) => `${l.label}: ${l.url}`).join('\n') : ''
  return `${greeting}\n\n${paragraphs}${links}\n\n${SENDER}`
}
...
const email = (params: {
  ...
  bodyHtml: buildEmailHtml({ paragraphs: params.paragraphs, links: params.links }),
  bodyText: buildEmailText({ paragraphs: params.paragraphs, links: params.links }),
})

const POLLS: Record<1 | 2 | 3 | 4 | 5 | 6, ProgrammePollReference> = {
  1: {
    question:
...
const POLL_URLS: Record<1 | 2 | 3 | 4 | 5 | 6, string> = {
...
const GAMMA_URLS: Record<1 | 2 | 3 | 4 | 5 | 6, string> = {
...
const PODCAST_URLS: Record<1 | 2 | 3 | 4 | 5 | 6, { podcast: string; workbook: string }> = {
  1: {
    podcast: 'https://youtu.be/zDtg3_l4SL8?si=RpiZZOKj29JtM3LD',
    workbook: 'https://canva.link/5lawehkji6pr7v8',
  },
  2: {
    podcast: 'https://youtu.be/1ICq3TkSbps?si=LxIcqgjxSkL7JTLd',
    workbook: 'https://canva.link/prhgdj6v1k9bqgd',
  },
  3: {
    podcast: 'https://youtu.be/Px_Ix4BO90k?si=rt4Z1FUsq5q2Iz5i',
    workbook: 'https://canva.link/hxuahu4qj9j52u6',
  },
  4: {
    podcast: 'https://youtu.be/qlBo-jmw-Wk?si=fkEJFGa3qyZxR1dv',
    workbook: 'https://canva.link/0tnaf277xtxztbm',
  },
  5: {
    podcast: 'https://youtu.be/wpyPoctC0vk?si=APtucPfAVVEFyb3V',
    workbook: 'https://canva.link/axd181pdemt18ze',
  },
  6: {
    podcast: 'https://youtu.be/Zg2LRLCHwtA?si=jFO5osbY3f-T36aY',
    workbook: 'https://canva.link/pvquc25ynqs9ubd',
  },
}

const GAMMA_SLIDES: Record<1 | 2 | 3 | 4 | 5 | 6, ProgrammeGammaSlide[]> = {
  1: [
    {
      title: 'Slide 1 | Why this prompt exists',
      body: `You cannot change a communication default you haven't named. Feedback from colleagues is usually too polite to be diagnostic. A structured prompt has no social cost.`,
    },
    {
      title: 'Slide 2 | The prompt (copy-paste)',
      body: `You are a communication coach for transformation leaders. I'll describe three recent high-stakes stakeholder moments -- a pitch, a pushback, and a decision ask. For each, identify: (1) what I said, (2) what I likely wanted to land, (3) the gap between the two, (4) which default pattern I slipped into (over-explaining, under-explaining, hedging, going corporate, pushing to close too early). Then name the one pattern across all three. Moments: [paste]`,
    },
    {
      title: 'Slide 3 | How to pick your three moments',
      body: `Pick moments you replayed afterwards. A meeting where the decision didn't land the way you expected. A 1:1 where you felt you talked past each other. A presentation where you know half the room wasn't with you. Those are the ones with data.`,
    },
    {
      title: 'Slide 4 | What to do with what you find',
      body: `Write the pattern on paper. One word. Keep it visible this week. Every stakeholder conversation this week, ask yourself in the first 30 seconds: am I doing the thing?`,
    },
  ],
  2: [
    {
      title: 'Slide 1 | Why a blueprint, not a journal',
      body: `You will not reread a journal. You will reread a one-pager pinned to your desk. The Connection Blueprint is the practical artefact of the communication work -- it needs to be short enough to live with you.`,
    },
    {
      title: 'Slide 2 | The prompt (copy-paste)',
      body: `You are a communication coach for transformation leaders. Based on what I share below, produce a one-page Connection Blueprint with five sections: (1) The default pattern I'm rewiring, (2) The replacement behaviour I'm practicing, (3) My three hardest stakeholders and what actually lands for each of them, (4) One scripted opener for high-stakes asks, (5) One metric I'll track for 30 days. Keep it shorter than you want to. Material: [paste default-pattern work, listening reflections, stakeholder map]`,
    },
    {
      title: 'Slide 3 | What to do with it',
      body: `Pin to your desktop. 30-day calendar reminder to review. At Week 6, revisit this alongside your Change Leader Playbook to see how your influence work shaped your change work.`,
    },
    {
      title: 'Slide 4 | If you skip this',
      body: `You leave Course 1 with frameworks and no blueprint. Frameworks fade. Blueprints stick because they fit in one glance. This is the twenty-minute difference between practitioners who change how they influence and practitioners who add "stakeholder management" to their LinkedIn.`,
    },
  ],
  3: [
    {
      title: 'Slide 1 | Why this prompt exists',
      body: `A vision statement that lands with leadership often doesn't land with the people who have to execute it. The Tired Pragmatist hears "another programme to deliver on top of my day job." The Quiet Cynic hears buzzwords. You need to hear those versions before the town hall, not after.`,
    },
    {
      title: 'Slide 2 | The prompt (copy-paste)',
      body: `You are a change communication coach. I'll paste a vision statement for a change initiative. Return four paraphrases -- how each of these archetypes actually hears it: (1) True Believer (already bought in, wants clarity), (2) Tired Pragmatist (focused on delivery, allergic to rhetoric), (3) Quiet Cynic (seen this before, quietly sceptical), (4) Exposed Middle Manager (knows they carry the execution weight). Then rewrite the vision in a version that lands with all four without losing the core message. Vision: [paste]`,
    },
    {
      title: 'Slide 3 | How to use the output',
      body: `Walk the original and the rewrite into your next three 1:1s. Ask which version lands. You'll get better calibration from three 15-minute conversations than from any consultant report.`,
    },
    {
      title: 'Slide 4 | What to do with the rewrite',
      body: `Test it in your next team meeting. Don't announce it's a revision. Just use it. Watch which of the four archetypes stays engaged. That's the data.`,
    },
  ],
  4: [
    {
      title: 'Slide 1 | Why this prompt exists',
      body: `Traditional roadmaps are Gantt charts. Gantt charts don't tell you that Week 5 is when the middle managers realise they lose their direct reports, or that Week 9 is when the first team's workflow gets re-mapped. The emotional pinch points drive adoption. Map them explicitly.`,
    },
    {
      title: 'Slide 2 | The prompt (copy-paste)',
      body: `You are a change management coach trained in emotional intelligence. I'll paste a change roadmap (weeks, milestones, who is affected). For each milestone, name: (1) the specific group most affected, (2) the likely emotional response (exposed, resistant, hopeful, checked out), (3) the leadership move that would defuse it ahead of time, (4) the conversation I should have at least two weeks before. Roadmap: [paste]`,
    },
    {
      title: 'Slide 3 | How to use the output',
      body: `Put the "conversation I should have two weeks before" column in your calendar. Literally. Book those 1:1s now. Most change leaders book them reactively, after the pinch has hit.`,
    },
    {
      title: 'Slide 4 | What to do if the output is too clean',
      body: `If every pinch point has a tidy leadership move, rerun the prompt with this addition: "Be more honest about where this roadmap is going to bruise people. Don't soften it." The second output is usually the useful one.`,
    },
  ],
  5: [
    {
      title: 'Slide 1 | Why behavioural KPIs beat output KPIs',
      body: `Output metrics (deliverables shipped, training attended) tell you what happened. Behavioural KPIs (are people asking different questions, making different decisions, pushing back differently) tell you whether the change actually stuck. The second one predicts the first, 12 months out.`,
    },
    {
      title: 'Slide 2 | The prompt (copy-paste)',
      body: `You are a change measurement coach. I'll paste a change initiative. Produce a behavioural KPI dashboard: for each of the three main stakeholder groups, name (1) one behaviour that would indicate the change is landing, (2) one behaviour that would indicate it is not, (3) the cadence to observe it (weekly, monthly), (4) where to observe it (meetings, decisions, workflow artefacts). Then give me one leading indicator -- the behaviour that shifts first when the change is working. Initiative: [paste]`,
    },
    {
      title: 'Slide 3 | How to use the output',
      body: `The leading indicator is the most valuable row. Track that one weekly. Track the rest monthly. If the leading indicator moves, the rest will follow in 6-8 weeks. If it doesn't move, you have a communication problem, not a measurement problem.`,
    },
    {
      title: 'Slide 4 | The adjacent prompt',
      body: `After you have the dashboard, rerun with this follow-up: "Based on these KPIs, what is the conversation I need to have with my exec sponsor in three months if the leading indicator hasn't moved?" Write that script now while you're unemotional about it.`,
    },
  ],
  6: [
    {
      title: 'Slide 1 | Why this matters more than the capstone',
      body: `The capstone is an input. A playbook is an output. Practitioners finish programmes with notes, not playbooks. Six weeks of work are worth a one-page playbook -- if you will actually reread it.`,
    },
    {
      title: 'Slide 2 | Before you run the prompt',
      body: `You need your Week 2 Connection Blueprint, your Week 3 vision stress-test output, your Week 4 EQ-mapped roadmap, your Week 5 behavioural KPI dashboard, your Change Leader Playbook Capstone, and notes from at least two peer sessions. Without those, the prompt has nothing to work with.`,
    },
    {
      title: 'Slide 3 | The prompt (copy-paste)',
      body: `You are a transformation coach. I'll paste six things: a Connection Blueprint, a vision stress-test, an EQ-mapped change roadmap, a behavioural KPI dashboard, a Change Leader Playbook, and peer session reflections. Synthesise them into a one-page 90-day plan. Three 30-day blocks. For each block: one communication shift I'm practicing, one stakeholder I'm engaging differently, one KPI I'm tracking, one conversation I'm having. Use my language, not template language. Shorter than you want to. Material: [paste]`,
    },
    {
      title: 'Slide 4 | What to do Monday morning',
      body: `15 minutes with your exec sponsor. Walk them through the playbook. Make one edit in the room based on their feedback. That's how playbooks become commitments. Email revisits are promises to yourself. Walk-throughs are promises to someone else.`,
    },
  ],
}

const AFFIRMATIONS: Record<1 | 2 | 3 | 4 | 5 | 6, { title: string; body: string }> = {
  1: {
    title: 'Welcome to the Power Journey',
    body: `Welcome, {{firstName}}. This week is about how you actually come across when it matters -- in a pitch, in a pushback, in a room where influence is earned and not given. Most leaders think they communicate clearly. Their teams know otherwise. This week we get honest about the gap.`,
  },
  2: {
    title: 'Influence is a practice',
    body: `Week 2, {{firstName}}. Last week you worked on fundamentals -- how people process information, tone, feedback. This week you put it under pressure: presenting, active listening, conflict communication, and the comms of sales. The Connection Blueprint Capstone closes the loop. Influence is not a talent. It's a practice.`,
  },
  3: {
    title: 'New course. New rhythm.',
    body: `Week 3, {{firstName}}. New course. New rhythm. Last course was about how you speak. This course is about what you're leading people through -- transformation that rewires how the business works. This week you learn to communicate the vision so people actually move, not just nod.`,
  },
  4: {
    title: 'The heaviest week',
    body: `Week 4, {{firstName}}. This is the heaviest week in the journey -- Change Strategies, Leading a Team Through Change, and Leveraging EQ, back-to-back. Change programmes fail on exactly these three: no roadmap, no team care, no emotional intelligence. This week you build all three.`,
  },
  5: {
    title: 'Behavioural KPIs, not feelings',
    body: `Week 5, {{firstName}}. Stakeholder management, conflict, problem solving, and measuring change impact. This is the unglamorous middle of transformation leadership -- the part where you learn to track what actually shifted versus what everyone claims shifted. Behavioural KPIs, not feelings.`,
  },
  6: {
    title: 'Final week',
    body: `Final week, {{firstName}}. Six weeks ago you showed up to work on how you influence and lead change. This week you synthesise it. The Change Leader Playbook pulls together vision, strategy, stakeholder work, continuous improvement, and storytelling into one operating system. Most practitioners finish a journey with frameworks. You finish with a playbook.`,
  },
}

const POLL_INBOX_BODIES: Record<1 | 2 | 3 | 4 | 5 | 6, string> = {
...
const PEER_NUDGES: Record<
  1 | 2 | 3 | 4 | 5 | 6,
  { title: string; body: string }
> = {
  1: {
    title: 'Peer session: Creative Problem Solving',
    body: `Your first peer session is waiting, {{firstName}}. Creative Problem Solving Session -- 30 minutes with a partner. Bring one communication breakdown from your last month at work. Not the easy one. The one where you walked away thinking "that didn't go how I wanted." Your partner helps you rewind and redesign it.`,
  },
  2: {
    title: 'Peer session: Stakeholder Mapping',
    body: `Stakeholder Mapping peer session is open, {{firstName}}. 30 minutes. Bring a list of the 5-7 stakeholders on your current initiative. Your partner walks you through who actually decides, who influences, who resists, and who you are overestimating your relationship with. Bring honesty. Nothing else is useful here.`,
  },
  3: {
    title: 'Peer session: Peer-to-Peer Ideation Workshop',
    body: `Peer-to-Peer Ideation Workshop is open, {{firstName}}. 30+ minutes. This one is different -- less reflection, more build. Bring one stuck problem on your change initiative. Your partner co-ideates with you. You leave with three options you would not have generated alone. No solo thinking in a transformation context.`,
  },
  4: {
    title: 'Peer session: Change Readiness Assessment',
    body: `Change Readiness Assessment peer session is open, {{firstName}}. 30 minutes. Bring your current change initiative and the team it's landing on. Your partner walks you through three questions: Are they informed? Are they capable? Are they willing? Whichever you rate lowest is where Week 5's work needs to go.`,
  },
  5: {
    title: 'Peer session: Feasibility Study Exercise',
    body: `Feasibility Study Exercise peer session is open, {{firstName}}. 30 minutes. Bring one proposed next step on your change initiative. Your partner pressure-tests the feasibility -- resources, dependencies, risk, timing. You leave with the plan de-risked or the plan in the bin. Either is a win.`,
  },
  6: {
    title: 'Peer session: Stakeholder Communication Plan',
    body: `The final peer session is open, {{firstName}}. Stakeholder Communication Plan -- 30 minutes to draft who you communicate what to, when, across the next 90 days. Your Week 2 Connection Blueprint, your Week 4 Change Readiness work, and your Week 6 Change Leader Playbook all feed into this. One plan. One page. Walk it out on Monday.`,
  },
}
...
const SURVEYS: Record<
  1 | 2 | 4 | 6,
  { title: string; body: string; week: 1 | 2 | 4 | 6; day: 3 | 6 }
> = {
  1: {
    title: 'Open the app: W1 Baseline Assessment',
    body: `{{firstName}}, the Art of Connection Baseline Assessment is inside the course this week. 10 minutes. Your answers become your reference point for Week 2. Open the app.`,
    week: 1,
    day: 3,
  },
  2: {
  ...
  4: {
    title: 'Open the app: W4 Leadership Self Assessment',
    body: `{{firstName}}, the Leadership Self Assessment for Leading Through Change is inside the course this week. Same drill as Week 1: 10 minutes, your reference point for Week 6. Open the app.`,
    week: 4,
    day: 3,
  },
  6: {
    title: 'Open the app: Final Course Feedback',
    body: `Final survey, {{firstName}}. Course Feedback for Leading Through Change is inside the course. 5 minutes. Last thing before graduation.`,
    week: 6,
    day: 6,
  },
}

const DIGITAL_EDGE_EMAILS: Record<1 | 2 | 3 | 4 | 5 | 6, SixWeekEmailContent> = {
  1: email({
    subject: 'The Claude prompt that maps your communication defaults in 10 minutes',
    preview:
    ...
    links: [{ label: 'Gamma', url: GAMMA_URLS[1] }],
  }),
  2: email({
    subject: 'Build your Connection Blueprint in 20 minutes',
    preview:
    ...
    links: [{ label: 'Gamma', url: GAMMA_URLS[2] }],
  }),
  3: email({
    subject: 'Stress-test your change vision before your team tunes it out',
    preview:
    ...
    links: [{ label: 'Gamma', url: GAMMA_URLS[3] }],
  }),
  4: email({
    subject: 'A change roadmap prompt that flags the EQ moments before they hit',
    preview:
    ...
    links: [{ label: 'Gamma', url: GAMMA_URLS[4] }],
  }),
  5: email({
    subject: 'Build a behavioural KPI dashboard for your change in 15 minutes',
    preview:
    ...
    links: [{ label: 'Gamma', url: GAMMA_URLS[5] }],
  }),
  6: email({
    subject: 'Your 90-day Change Leader Playbook (final Digital Edge)',
    preview:
    ...
    links: [{ label: 'Gamma', url: GAMMA_URLS[6] }],
  }),
}

const SHAMELESS_PODCAST_EMAILS: Record<1 | 2 | 3 | 4 | 5 | 6, SixWeekEmailContent> = {
  1: email({
    subject: 'Weekend listen: the pitch that finally landed after 18 months',
    preview: '30-60 min podcast + workbook to complete. Pairs with Mastering Communication Fundamentals.',
    paragraphs: [
    ...
    links: [
      { label: 'Podcast', url: PODCAST_URLS[1].podcast },
      { label: 'Workbook Playbook', url: PODCAST_URLS[1].workbook },
    ],
  }),
  2: email({
    ...
    links: [
      { label: 'Podcast', url: PODCAST_URLS[2].podcast },
      { label: 'Workbook Playbook', url: PODCAST_URLS[2].workbook },
    ],
  }),
  3: email({
    subject: 'Weekend listen: the vision that sounded great and changed nothing',
    preview: '30-60 min podcast + workbook. Pairs with Communicating a Vision.',
    paragraphs: [
    ...
    links: [
      { label: 'Podcast', url: PODCAST_URLS[3].podcast },
      { label: 'Workbook Playbook', url: PODCAST_URLS[3].workbook },
    ],
  }),
  4: email({
    subject: 'Weekend listen: the EQ moment that saved the programme',
    preview: '30-60 min podcast + workbook. Pairs with Leading a Team Through Change and Leveraging EQ.',
    paragraphs: [
    ...
    links: [
      { label: 'Podcast', url: PODCAST_URLS[4].podcast },
      { label: 'Workbook Playbook', url: PODCAST_URLS[4].workbook },
    ],
  }),
  5: email({
    subject: 'Weekend listen: what they measured, what mattered, and the gap between them',
    preview: '30-60 min podcast + workbook. Pairs with Measuring Change Impact.',
    paragraphs: [
    ...
    links: [
      { label: 'Podcast', url: PODCAST_URLS[5].podcast },
      { label: 'Workbook Playbook', url: PODCAST_URLS[5].workbook },
    ],
  }),
  6: email({
    subject: 'Final weekend listen: what actually stuck',
    preview: '30-60 min podcast + workbook. Closes Leading Through Change and Continuous Improvement. Closes the Power Journey.',
    paragraphs: [
    ...
    links: [
      { label: 'Podcast', url: PODCAST_URLS[6].podcast },
      { label: 'Workbook Playbook', url: PODCAST_URLS[6].workbook },
    ],
  }),
}

const buildWeekTemplates = (week: 1 | 2 | 3 | 4 | 5 | 6): SixWeekNotificationTemplate[] => {
  const templates: SixWeekNotificationTemplate[] = []
  ...
  templates.push({
    ...
    referenceContent: { gammaSlides: GAMMA_SLIDES[week] },
    targetAudience: TARGET_AUDIENCE,
    isActive: true,
  })
...
const SURVEY_TEMPLATES: SixWeekNotificationTemplate[] = (
  [1, 2, 4, 6] as const
).map((surveyWeek) => ({
  key: `tb6w_w${surveyWeek}_d${SURVEYS[surveyWeek].day}_survey`,
  programme: PROGRAMME,
  week: SURVEYS[surveyWeek].week,
  day: SURVEYS[surveyWeek].day,
  contentKind: 'survey_reminder' as const,
  channel: 'push' as const,
  title: SURVEYS[surveyWeek].title,
  messageBody: SURVEYS[surveyWeek].body,
  targetAudience: TARGET_AUDIENCE,
  isActive: true,
}))

export const TRANSFORMING_BUSINESS_6W_NOTIFICATIONS: readonly SixWeekNotificationTemplate[] =
  Object.freeze([
...
export const getSixWeekNotificationTemplate = (
  key: string,
): SixWeekNotificationTemplate | undefined =>
  TRANSFORMING_BUSINESS_6W_NOTIFICATIONS.find((template) => template.key === key)
