...
exports.checkStalePendingReferrals = exports.creditReferralPointsCallable = exports.onReferredUserFirstActivity = void 0;
 ...
 */
function getWindowNumber(weekNumber) {
 ...
 */
exports.onReferredUserFirstActivity = functions
    .region("us-central1")
    .firestore.document("pointsLedger/{ledgerId}")
    .onCreate(async (snapshot, context) => {
    const ledgerData = snapshot.data();
    const referredUid = ledgerData.uid;
    console.log(`[Referral] New ledger entry for user ${referredUid}, activity: ${ledgerData.activityId}`);
    ...
    try {
        ...
        if (referralData.status !== "pending") {
            console.log(`[Referral] Referral status is ${referralData.status}, not pending`);
            return null;
        }
        ...
        console.log(`[Referral] First activity detected for referred user ${referredUid}, crediting referrer ${referrerUid}`);
        // Award points to referrer
        return await creditReferralPointsInternal(referredUid, referrerUid, ledgerData.activityId);
    }
    catch (error) {
 ...
 */
exports.creditReferralPointsCallable = functions
    .region("us-central1")
    ...
    if (callerRole !== "super_admin" && callerRole !== "partner") {
        throw new functions.https.HttpsError("permission-denied", "Only super admins and partners can manually credit referral points");
    }
    const { referredUid } = data;
    if (!referredUid) {
        throw new functions.https.HttpsError("invalid-argument", "referredUid is required");
    }
    try {
        ...
        await creditReferralPointsInternal(referredUid, referrerUid, "manual_credit");
        console.log(`[Referral] Manual credit by ${callerUid} for referral: ${referredUid} -> ${referrerUid}`);
        return { success: true, message: "Referral points credited successfully" };
    }
    catch (error) {
        console.error(`[Referral] Manual credit failed:`, error);
        throw new functions.https.HttpsError("internal", `Failed to credit referral points: ${error}`);
    }
});
/**
 ...
 */
async function creditReferralPointsInternal(referredUid, referrerUid, triggerActivityId) {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (referralData.status !== "pending") {
            console.log(`[Referral] Referral status changed to ${referralData.status}, skipping`);
            return;
        }
        ...
        if (currentReferralCount >= REFERRAL_MAX_PER_USER) {
            console.log(`[Referral] Referrer ${referrerUid} has reached max referrals (${REFERRAL_MAX_PER_USER})`);
            tx.update(referralRef, {
                status: "rejected",
                creditedAt: timestamp,
        ...
        console.log(`[Referral] Successfully credited ${REFERRAL_POINTS} points to ${referrerUid} for referring ${referredUid}`);
    });
 ...
 */
exports.checkStalePendingReferrals = functions
    .region("us-central1")
    ...
    try {
        const stalePendingQuery = await db
            ...
            .get();
        console.log(`[Referral] Found ${stalePendingQuery.size} stale pending referrals`);
        let processed = 0;
        for (const doc of stalePendingQuery.docs) {
            ...
            await db.collection("notifications").add({
                ...
                message: "Your referred user hasn't completed their first activity yet. Encourage them to get started!",
                actionUrl: "/app/referral-rewards",
                isRead: false,
                createdAt: admin.firestore.FieldValue.serverTimestamp(),
            });
...
//# sourceMappingURL=referral-points.js.map